	...
	$return .= '<div class="kwayy-meta-details">';
		if ( 'post' == get_post_type() ) {
			if( !is_single() ){
				...
				$return .= sprintf( '<div class="date-time-album"><span class="date-time-album-child"><time class="entry-date" datetime="%1$s" >%2$s <span class="album-month  album-year">%3$s <span class="album-year">%4$s</span></span></time></span></div>',
                    get_the_date( 'c' ),
                    get_the_date( 'j' ),
                    get_the_date( 'M' ),
                    get_the_date( ', Y' ),
