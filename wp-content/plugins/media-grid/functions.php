...
function mg_get_soundcloud_embed($track_url, $inline = false, $lazyload = false) {
	...
	return '<iframe class="mg_soundcloud_embed" scrolling="no" frameborder="no" '.$lazyload_code.'//w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/tracks/'.$track->id.'&amp;color=ff5500&amp;auto_play='.$autoplay.'&amp;hide_related=true&amp;show_artwork=true'.$inline_visual.'" '.$z_index.'></iframe>';
}
...
function mg_types_meta_opt($type) {
	...
	// static image & inlie audio
	if($type == 'simple_img') {
		$opt_arr = array(
			array(
				'label' 	=> __('Display overlay?', 'mg_ml'),
				'name'		=> 'mg_static_show_overlay',
				'descr'		=> __('If checked displays overlay for this item', 'mg_ml'),
				'type' 		=> 'checkbox',
	...
	elseif($type == 'img_gallery') {
		$opt_arr = array(
			...
			array(
				'type' 		=> 'empty',
				'validate'	=> array('index'=>'mg_slider_vid', 'label'=>'Slider Videos')
			)
		);
	}
	...
	elseif($type == 'video' || $type == 'inl_video') {
		$opt_arr = array(
			...
			array(
				'type' 		=> 'empty',
				'validate'	=> array('index'=>'mg_video_use_poster', 'label'=>'Use featured image as poster')
			),
			array(
				'type' 		=> 'empty',
				'validate'	=> array('index'=>'mg_autoplay_inl_video', 'label'=>'Autoplay inline video')
			)
		);
	}
	...
	// validation trick for inline audio
	elseif($type == 'inl_audio') {
		$opt_arr = array(
			  array(
				  'type' 		=> 'empty',
				  'validate'	=> array('index'=>'mg_static_show_overlay', 'label'=>'Show overlay?')
			  ),
			  array(
				  'type' 		=> 'empty',
				  'validate'	=> array('index'=>'mg_soundcloud_url', 'label'=>'Soundcloud URL')
	...
	// post contents
	elseif($type == 'post_contents') {
		$opt_arr = array(
			array(
				'label' 	=> __('Post type and taxonomy', 'mg_ml'),
				'name'		=> 'mg_cpt_source',
				'descr'		=> __('Choose the post type and taxonomy to fetch the post from', 'mg_ml'),
				'type' 		=> 'select',
				'options'	=> mg_get_cpt_with_tax(),
				'class'		=> 'mg_cpt_source',
				'validate'	=> array('index'=>'mg_cpt_source', 'label'=>'CPT and taxonomy')
			),
			array(
				'label' 	=> __("Taxonomy's term", 'mg_ml'),
				'name'		=> 'mg_cpt_tax_term',
				'descr'		=> __("Choose the taxonomy's term to fetch the post from", 'mg_ml'),
				'type' 		=> 'select',
				'options'	=> mg_get_taxonomy_terms( mg_get_cpt_with_tax(true) ),
				'class'		=> 'mg_cpt_tax_term',
				'validate'	=> array('index'=>'mg_cpt_tax_term', 'label'=>"Taxonomy's term")
			),
			array(
				'label' 	=> __('Query offset', 'mg_ml'),
				'name'		=> 'mg_post_query_offset',
				'descr'		=> __('Set how many posts to skip during the query', 'mg_ml'),
				'type' 		=> 'slider',
				'min_val' 	=> '0',
				'max_val'	=> '20',
				'step' 		=> '1',
				'value' 	=> '',
				'def' 		=> 0,
				'validate'	=> array('index'=>'mg_post_query_offset', 'label'=>'Query offset')
			),
			array(
				'label' 	=> __('Hide featured image in lightbox?', 'mg_ml'),
				'name'		=> 'mg_hide_feat_img',
				'descr'		=> __("If checked, hides posts featured image in lightbox", 'mg_ml'),
				'type' 		=> 'checkbox',
				'validate'	=> array('index'=>'mg_hide_feat_img', 'label'=>'Hide featured image?')
			),
			array(
				'label' 	=> __('Direct link to post?', 'mg_ml'),
				'name'		=> 'mg_link_to_post',
				'descr'		=> __('If checked, turns the item into a direct link', 'mg_ml'),
				'type' 		=> 'checkbox',
				'validate'	=> array('index'=>'mg_link_to_post', 'label'=>'Direct link to post?')
			),
		);
	}
	...
	elseif($type == 'spacer') {
		$opt_arr = array(
			/*array(
				'label' 	=> __('Fixed width', 'mg_ml'),
				'name'		=> 'mg_spacer_fix_w',
				'descr'		=> __('Set a fixed width - set zero to use normal item sizing', 'mg_ml'),
				'type' 		=> 'slider',
				'min_val' 	=> '0',
				'max_val'	=> '500',
				'step' 		=> '5',
				'value' 	=> 'px',
				'def' 		=> 0,
				'validate'	=> array('index'=>'mg_spacer_fix_w', 'label'=>'Spacer fixed width')
			),
			array(
				'label' 	=> __('Fixed height', 'mg_ml'),
				'name'		=> 'mg_spacer_fix_h',
				'descr'		=> __('Set a fixed height - set zero to use normal item sizing', 'mg_ml'),
				'type' 		=> 'slider',
				'min_val' 	=> '0',
				'max_val'	=> '500',
				'step' 		=> '5',
				'value' 	=> 'px',
				'def' 		=> 0,
				'validate'	=> array('index'=>'mg_spacer_fix_h', 'label'=>'Spacer fixed height')
			),*/
			array(
				'label' 	=> __('Spacer visibility', 'mg_ml'),
				'name'		=> 'mg_spacer_vis',
				'descr'		=> __('Set spacer visibility in grid modes', 'mg_ml'),
				'type' 		=> 'select',
				'options'	=> array('' => __('always visible', 'mg_ml'), 'hidden_desktop' => __('hidden by default', 'mg_ml'), 'hidden_mobile' => __('hidden on mobile', 'mg_ml')),
				'validate'	=> array('index'=>'mg_spacer_vis', 'label'=>'Spacer visibility')
			)
		);
	}
	else {return false;}
...
function mg_meta_opt_generator($type, $post) {
	...
	foreach($opt_arr as $opt) {
		if($opt['type'] != 'empty') {
			...
			$class = (isset($opt['class'])) ? 'class="'.$opt['class'].'"' : '';
...
function mg_post_contents_get_post($item_id) {
	...
	$args = array(
		...
		'post_status' => 'publish', 
		'posts_per_page' => 1,
		'offset' => get_post_meta($item_id, 'mg_post_query_offset', true),
		'meta_query' => array( 
...
function mg_grid_terms_data($terms, $return = 'html', $filters_align = 'top', $selected = false, $hide_all = false) {
	...
	if(!$terms) { return false; }
		...
		elseif($return == 'dropdown') {
			...
			foreach($terms_data as $term) {
				...
				$grid_terms_list .= $true_sep.'<a rel="'.$term['id'].'" class="mgf_id_'.$term['id'].' '.$sel.' mgf" href="javascript:void(0)">'.$pre_icon.$term['name'].$after_icon.'</a>';
